...
const group = await p.group(
  {
    ...
    linkToworkingWith: () => p.text({ message: "Linkt to what you're working with?" }),
    repo: () => {
      return p.select({
        message: "Source code",
        options: [
...
async function getRepo() {
  try {
    ...
    const repo = stdout.split("\n")[0].replace(/\s+/g, " ").trim().split(" ")[1]

    return repo
  } catch (error) {
    if (error instanceof Error) console.error("getRepo ~ error:", error.message)
    return ""
  }
}
...
working with: ${group.linkToworkingWith ? `[${group.workingWith}](${group.linkToworkingWith})` : group.workingWith}
...
function getHours(hours: string) {
  if (Math.floor(Number.parseFloat(hours)) === 1) return "hour"
  return "hours"
}
...
try {
  ...
  await fs.writeFile(`${DAYS_REPO_PATH}/.${today}`, commit)
